...
var findPath = require('./helpers').findPath;

module.exports = exports = model_jsonSchema
 ...
 */
function model_jsonSchema(fields, populate, readonly, removeRequired) {
  ...
  if (removeRequired) __removeRequired(jsonSchema);
...
function __delPath(jsonSchema, path) {
  var jss = findPath(jsonSchema, path);
...
function __excludedPaths(schema, selection) {
  ...
  var paths = __allPaths(schema);
  var exclude = paths.reduce((excl, p) => {
    var path = __getPath(schema, p);
...
function __excludedReadonlyPaths(schema, rules, prefix) {
  ...
  Object.keys(props).forEach( f => {
    ...
    if (props[f].type === 'object' || props[f].type === 'array')
      return __excludedReadonlyPaths(
        props[f], rules, prefix + f + '.'
      );
  });
